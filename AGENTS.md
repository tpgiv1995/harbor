...
**The premise:** Harbor was developed on one machine (Linux originally; one
Windows machine since 2026-08-10), and your user's setup is not that machine. Different config homes, different accounts,
...
```sh
node -v                                  # must be >= 22
uname -s                                 # which OS; see section 11 for platform status
...
command -v claude codex cursor-agent 2>/dev/null
```
...
```powershell
node -v                                            # must be >= 22
Get-ChildItem $env:USERPROFILE -Filter ".claude*"  # Claude config homes = "plans" in Harbor
(Get-ChildItem "$env:USERPROFILE\.claude\projects" -ErrorAction SilentlyContinue).Count  # existing sessions
Get-Command claude, codex, cursor-agent -ErrorAction SilentlyContinue
```
...
# Windows (PowerShell)
Get-Item "$env:USERPROFILE\.harbor\config.json"
```

On Windows the shared config file is `%USERPROFILE%\.harbor\config.json`,
deliberately outside `%APPDATA%`: agent shells (Claude Code included) run in an
MSIX container whose `%APPDATA%` is a per-launcher virtualized mirror, so a
file found or "fixed" there is not the file the real app reads, and `%APPDATA%`
must not be trusted from an agent shell.
...
| Variable | Meaning |
|---|---|
| `HARBOR_SESSION_BACKEND` | `sessiond` (Harbor's own daemon, and the default when unset) is the only value. `herdr` throws a retirement error (that backend was retired 2026-08-14), and any other value refuses to start. |
...
| `HARBOR_HERDR_BIN`, `HARBOR_HERDR_DIR`, `HARBOR_HERDR_UNIT` | Legacy names, still read by `resolveUnitPolicy` in `bin/harbor-bin.cjs` as isolation signals only (a relocated install may not manage the default systemd unit); no backend consumes them since the Herdr retirement (2026-08-14) |
...
**Windows** is the platform Harbor runs and is developed on, validated end to
end: the full app (Electron GUI included) has run there since 2026-08-11, the
unit gate is green, and CI runs it on `windows-latest` (since 2026-08-14).
Detail in `setup/windows/README.md`. Fixes already made, so you do not
...
never pass; and `node-pty` refuses a signal argument to `kill`. Remaining gaps:
the `daemon` and `bin` real-pty test families are excluded from CI while their
Windows ports are tracked separately, and the e2e suite currently runs on no
machine pending its Windows offscreen-window port.

**Linux** was the original development platform; that install was retired
...
Run `npm test` from `app/`. The end-to-end gate (`npm run test:e2e`) currently
runs on no machine, pending its Windows port; when it returns, it must pass
**twice consecutively**, because a single green run is not accepted: the
...
Its sequel, 2026-08-08: **measure the COST OF A REPEAT, not the cost of one
interaction.** Every spec drove each action once, so nothing could see that one
iOS `visualViewport` scroll event cost one `session:menu-state` RPC. A phone
fires those at the display refresh rate, so ~110 a second overflowed the
server's per-client queue, and Pat's connection died every minute or two for a
whole night. The specs that catch this shape assert a RATE (40 no-op events must
cost at most 3 calls) and are two-sided (the poll must still poll), because
"zero calls" is also what a feature that has stopped working looks like.

And **make the harness reproduce the case the product actually meets.** The
mobile fixture published a launched session synchronously inside the launch
itself, so a client that polled for it succeeded on its first try and the
twelve-second poll it really performs never ran once in the gate. Pointed at the
ordinary case (a session not listed yet), the same spec showed the old client
taking 29 seconds and then opening the WRONG session.
