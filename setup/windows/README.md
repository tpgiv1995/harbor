...
**Status: Harbor runs on Windows, GUI included, and Windows is the platform it
is developed on.** The full app has run there since 2026-08-11, the unit gate
is green, and CI runs it on `windows-latest` (since 2026-08-14). The core
session lifecycle (install, `node-pty` with a real ConPTY binding, daemon
start, spawning a session, sending input, reading the screen, closing a
session) was proven directly on 2026-08-07 (see the third update below and
`AGENTS.md` §11). The rest of this document, including the checklist below,
predates that progress and is kept for its real, verbatim output.
...
> `SUPPORTED_PROTOCOLS` in `app/src/main/herdr/client.js` (historical: that
> client and its protocol allowlist were deleted in the 2026-08-14 Herdr
> retirement; the principle lives on in the backend factory's exact-match
> check). So the specific
...
> GUI, which at the time of this update had not yet been launched on
> Windows; it has run there since 2026-08-11): daemon start,
...
> Still open at the time of this update: many unit tests failing on POSIX
> assumptions (`/proc`, file modes, the PATH executable bit), one hanging
> suite, and the Electron GUI not yet launched on Windows. All three have
> since been cleared: the GUI runs there since 2026-08-11, and the Windows
> unit failures were cleared before the CI flip to `windows-latest` on
> 2026-08-14. Full statement: `AGENTS.md` §11
...
2. **Move Harbor to accept protocol 17 / Herdr 0.7.5-preview, as an explicit
   ...
   check became `SUPPORTED_PROTOCOLS = [16, 17]`
   ...
   accepted. (Historical: the herdr client and its protocol allowlist were
   deleted in the 2026-08-14 retirement of that backend; the fail-closed
   exact-match posture lives on in the backend factory's check.) This removed the specific blocker described above. It did **not**
...
closing a session are proven there, directly against the daemon. The Electron
GUI itself has since been closed out too: the full app runs on Windows since
2026-08-11, the unit gate is green, and CI runs on `windows-latest`.
