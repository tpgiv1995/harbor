...
| `HARBOR_SESSIOND_SLICE` | `harbor-sessions.slice` (per-session memory containment; see below) |
| `HARBOR_SESSIOND_SESSION_MEMORY_HIGH` / `_MAX` / `_SWAP_MAX` | `8G` / `12G` / `2G` per session scope |
| `HARBOR_SESSIOND_SPAWN_TIMEOUT_MS` | `7000` (keeper start deadline; a spawn that misses it is reaped) |
| `HARBOR_SESSIOND_RECONCILE_INTERVAL_MS` | `60000` (within-boot dead-session sweep; `0` disables) |
| `HARBOR_SESSIOND_SCOPE_RETRY_MS` | `60000` (re-probe cooldown when systemd was unavailable) |

**Relocating the store disables memory containment unless you also name a
slice.** `HARBOR_SESSIOND_DIR`/`HARBOR_SESSIOND_SOCKET` without
`HARBOR_SESSIOND_SLICE` runs every session in the daemon's own cgroup, the
pre-2026-08-09 behaviour that let one runaway take the whole pool: correct for
a test harness (which must never write into the real slice), wrong for a real
install. If you relocate a production store (for example for the AF_UNIX
socket-path length limit), set `HARBOR_SESSIOND_SLICE=harbor-sessions.slice`
as well. `harbor-sessiond status` reports the truth either way in its `scopes`
field.
