...
| Variable | Meaning |
| --- | --- |
| `HARBOR_SESSION_BACKEND` | `sessiond` (the default when unset, Harbor's own daemon, no separate install) is the only backend. `herdr` throws a retirement error (that backend was retired 2026-08-14), and any other value refuses to start. |
