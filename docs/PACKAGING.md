...
runs `npm ci` and `npm test` (the unit suite only, excluding the `daemon` and
`bin` real-pty families) on `windows-latest`. (The step that installed the
pinned Herdr 0.7.4 CLI for the `test/herdr/` suite is gone: that backend, its
scripts and its suites were deleted in the 2026-08-14 retirement.) It deliberately does
