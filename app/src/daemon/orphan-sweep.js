...
const DEFAULT_PATTERNS = [
  ...
  // TEST FIXTURE SERVERS (2026-09-04). Ten python servers (eight
  // `mock_server.py` from screenshot drives, two `python -m http.server`)
  // sat on the machine all evening, each spawned by a session's bash tool
  // whose shell had long since exited. They are managed in the sense that
  // matters: a session started them, nothing else ever will, and a session
  // never means them to outlive its own shell. Pattern scope stays narrow on
  // purpose: a dev server someone starts by hand (`render.py --serve`) is not
  // a fixture and is not matched, and the parent-dead and two-sighting fences
  // still apply to everything here.
  /(^|[\\/\s"])mock_server\.py(\s|"|$)/i,
  /python(\d+(\.\d+)?)?(\.exe)?"?\s+(-u\s+)?-m\s+http\.server(\s|$)/i,
];
