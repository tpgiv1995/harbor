...
// The dialog's own rows after a question's options, in the CLI's numbering:
// "Type something." is options.length + 1 and "Chat about this" is
// options.length + 2 (measured 2026-09-03, both questions of a real batch:
// three options, row 4 types, row 5 chats). The card uses these for a question
// that is NOT on screen; for the one that is, the pty's own rows win.
const CHAT_ROW_RE = /^chat about this$/i;
function extraRowNumbers(question) {
  const count = Array.isArray(question?.options) ? question.options.length : 0;
  return { textIndex: count + 1, chatIndex: count + 2 };
}

// Second signal for WHICH question is on screen: the question line the pty
// drew, against each question's text. Weaker than option evidence (a label is
// a row, a question is prose the pane may wrap or clip), so it is consulted
// only when no option matched, and it needs enough surviving text to mean
// something. A hit on exactly one question is a hit; anything else is silence.
const MIN_QUESTION_KEY = 12;
// Among questions that tie on words, the dialog's own shape (single- or
// multi-select, which the pane draws as checkboxes) breaks the tie; two
// questions identical in words AND shape stay ambiguous.
function breakTie(questions, hits, multiSelect) {
  if (hits.length === 1) return hits[0];
  if (hits.length > 1 && typeof multiSelect === 'boolean') {
    const shaped = hits.filter((i) => Boolean(questions[i].multiSelect) === multiSelect);
    if (shaped.length === 1) return shaped[0];
  }
  return -1;
}

function matchQuestionText(questions, ptyQuestion, multiSelect) {
  if (!Array.isArray(questions) || questions.length === 0) return -1;
  const seen = matchKey(ptyQuestion);
  if (seen.length < MIN_QUESTION_KEY) return -1;
  const hits = [];
  questions.forEach((question, i) => {
    const key = matchKey(question.question);
    if (!key || key.length < MIN_QUESTION_KEY) return;
    if (key === seen || key.includes(seen) || seen.includes(key)) hits.push(i);
  });
  return breakTie(questions, hits, multiSelect);
}

// Which question the dialog is showing, strongest evidence first: the option
// rows, then the question line. -1 when neither is decisive.
function currentQuestionIndex(questions, menu) {
  if (!menu || !Array.isArray(menu.options)) return -1;
  const byOptions = matchQuestionIndex(questions, menu.options, menu.multiSelect);
  if (byOptions >= 0) return byOptions;
  return matchQuestionText(questions, menu.question, menu.multiSelect);
}

// Whether the strip says a question is answered. The strip's tab labels are
// the headers, truncated ("Chart win…"), so a header is matched by prefix
// either way; an absent strip, or a header with no tab, answers null (unknown),
// never false. `☒` is the mark, see menu-parse.js.
function answeredFromStrip(tabs, header, index = -1) {
  if (!Array.isArray(tabs) || !header) return null;
  const want = matchKey(header);
  if (!want) return null;
  const agrees = (tab) => {
    const seen = matchKey(tab.label);
    return Boolean(seen) && (seen === want || want.startsWith(seen) || seen.startsWith(want));
  };
  const questionTabs = tabs.filter((tab) => !tab.submit);
  // The strip lists questions in order, so the tab at the question's own
  // position is the first candidate; two headers sharing a prefix ("Delivery",
  // "Delivery method") would otherwise both read the first tab (review-caught
  // 2026-09-03).
  const positional = index >= 0 ? questionTabs[index] : null;
  if (positional && agrees(positional)) return Boolean(positional.done);
  const exact = questionTabs.find((tab) => matchKey(tab.label) === want);
  if (exact) return Boolean(exact.done);
  const byPrefix = questionTabs.filter(agrees);
  return byPrefix.length === 1 ? Boolean(byPrefix[0].done) : null;
}

// Is the screen the batch's final review ("Ready to submit your answers?").
// Its options are the CLI's own, never a question's, so no merge applies. The
// sentinel is what makes it the review; the labels alone are not, since a real
// question may well offer "Submit answers" and "Cancel" (review-caught
// 2026-09-03). A footerless menu is accepted for hand-built payloads.
function isReviewMenu(menu) {
  if (!menu) return false;
  if (!(menu.submitReview === true || menu.footer === '')) return false;
  const labels = (menu.options || []).map((option) => matchKey(option.label));
  return labels.includes('submit answers') && labels.includes('cancel');
}

function matchQuestionIndex(questions, ptyOptions, multiSelect) {
  if (!Array.isArray(questions) || questions.length === 0) return -1;
  ...
  if (best <= 0) return -1;
  const hits = scores.map((score, i) => (score === best ? i : -1)).filter((i) => i >= 0);
  return breakTie(questions, hits, multiSelect); // a tie on words is broken by shape, or not at all
}
...
function mergeAsk(menu, ask) {
  ...
  // The review screen belongs to the batch but shows no question: it is
  // labelled as such so the card can render it as the batch's last step, and
  // its options are left exactly as the pty drew them.
  if (isReviewMenu(menu)) {
    return {
      ...menu,
      review: true,
      asked: questions,
      batch: questions.length > 1
        ? {
          ...
          headers: questions.map((question, i) => ({
            ...
            current: false,
            answered: answeredFromStrip(menu.tabs, question.header, i),
          })),
        }
        : null,
    };
  ...
  const byOptions = matchQuestionIndex(questions, menu.options, menu.multiSelect);
  const index = byOptions >= 0 ? byOptions : matchQuestionText(questions, menu.question, menu.multiSelect);
  ...
  return {
    ...
    matchedBy: byOptions >= 0 ? 'options' : 'question',
    header: current.header || '',
    multiSelect: current.multiSelect,
    options: merged,
    // The whole question set, so the card can lay out every question of the
    // batch (the answer sheet, 2026-09-03) and not only the one on screen.
    asked: questions,
    ...
    // `answered` is the strip's own `☒`, null when the strip is not on screen.
    batch: questions.length > 1
      ? {
        count: questions.length,
        currentIndex: index,
