...
const MENU_KEYS = Object.freeze({
  down: '\x1b[B',
  up: '\x1b[A',
  // Question navigation in a BATCHED AskUserQuestion. Measured against the real
  // dialog on 2026-08-09: → and Tab both move to the next question, ← and
  // Shift+Tab both move to the previous one, while ↑/↓ stay on the OPTIONS.
  // The strip draws its own "←  ☐ Alpha  ☐ Beta  ✔ Submit  →" pager, which is
  // the affordance those two keys belong to.
  right: '\x1b[C',
  left: '\x1b[D',
  enter: '\r',
  cancel: '\x1b',
...
// The last screen of a batched AskUserQuestion: a review of the answers so far
// and "1. Submit answers / 2. Cancel". It draws NO footer at all (measured
// 2026-08-09, fixture real-batch-submit-review-120x60.txt), so it parsed as
// nothing and the final step of every batch fell through to the raw-screen
// fallback panel: answerable, but presented as a shape Harbor had never seen.
const SUBMIT_SENTINEL_RE = /Ready to submit your answers\?/i;
const FOOTERLESS_SENTINEL_RE = new RegExp(`${RESUME_SENTINEL_RE.source}|${SUBMIT_SENTINEL_RE.source}`, 'i');
...
//
// `☒` IS the answered mark (measured 2026-09-03 against Claude Code v2.1.258:
// answering question 1 redraws its tab as "☒ Finish approach", and the first
// tick on a multi-select flips its tab the same way; fixtures
// real-batch-one-answered-2.1.258.txt and real-batch-two-answered-2.1.258.txt).
// It was missing from this list, so a strip with two answered tabs and the
// Submit tab split into ONE segment, fell below the two-tab floor, and the
// whole strip line was read as the question: "←  ☒ Finish approach  ☒ Delivery
// ✔ Submit  → How should I hand off…". The 2026-08-09 multiselect fixture
// carried a `☒` too; nobody looked.
const TAB_GLYPHS = '☐☑☒✔✓⬜⬚';
...
// A message queued behind a dialog, echoed under it: a pointer WITH text on the
// same line. A composer's pointer stands alone (or behind a divider).
const QUEUED_ECHO_RE = /^\s{0,4}❯\s+\S/;
...
function parseMenu(screen) {
  ...
  const resumeSentinelLine = lines.findIndex((line) => FOOTERLESS_SENTINEL_RE.test(line));
  ...
  //
  // The FOOTER alone cannot answer this, which is what 2026-08-09 established
  // by driving a real multiSelect question: its footer is the ordinary "Enter
  // to select · Tab/Arrow keys to navigate · Esc to cancel" and never mentions
  // Space at all, so a footer-only test called it single-select and the card
  // drew numbered rows for a question you tick. The rows themselves are the
  // structural evidence and cannot be re-worded: only a multi-select question
  // draws a per-row checkbox. That is decided below, once the options exist.
  const footerZone = lines.slice(Math.max(0, footerIndex - 1), footerIndex + 3).join('\n');
  const footerSaysToggle = /space to (toggle|select)/i.test(footerZone);
  ...
  if (resumeDialog) {
    ...
    // A footerless dialog is only live when nothing but blank lines follows its
    // last option, OR when what follows is a message QUEUED while the dialog was
    // up: the CLI echoes it under the dialog as "❯ text" lines (live-caught
    // 2026-07-20 for footered dialogs; self-captured 2026-09-01 for the batch's
    // review screen, which parsed as unrecognized and dropped Pat's final
    // Submit into the raw-screen panel). A composer is never mistaken for that
    // echo: a composer draws a full-width divider directly above its "❯", the
    // echo has none, and a lone "❯" carries no text at all.
    const after = lines.slice(lastOptionLine + 1);
    const firstText = after.findIndex((line) => line.trim());
    if (firstText >= 0) {
      const echoLine = after[firstText];
      const dividerAbove = firstText > 0 ? isDivider(after[firstText - 1]) : isDivider(lines[lastOptionLine]);
      if (!QUEUED_ECHO_RE.test(echoLine) || dividerAbove) return null;
      // Everything under the echo must BE the echo: its own wrapped
      // continuation (indented) or another queued line. Working-turn chrome, a
      // composer, or prose at the margin means the review above is a ghost
      // (review-caught 2026-09-03: a stale review over "✶ Working… / esc to
      // interrupt" parsed as live).
      for (const line of after.slice(firstText + 1)) {
        if (!line.trim()) continue;
        if (isDivider(line) || /^\s*❯\s*$/.test(line) || /esc to interrupt/i.test(line)) return null;
        if (line.length - line.trimStart().length < 2) return null;
      }
    }
  }
  ...
  // A multi-select question's CONFIRM ROW ("Submit", or "Next" when more
  // questions follow) is drawn directly under the last option, unnumbered and
  // with no blank line before it, so the block above swallowed it and the
  // grouping attached it to that option as a DESCRIPTION: the card rendered the
  // bare word "Next" as the hint under the free-text field
  // (real-batch-multiselect-120x60.txt, caught in review 2026-08-09). It
  // belongs to the question, not to any option, and it is deliberately matched
  // as a whole line so an option whose text merely contains the word is safe.
  const CONFIRM_ROW_RE = /^(submit|next)$/i;
  const region = lines.slice(firstOptionLine, regionEnd + 1);
  // The pointer can sit ON the unnumbered confirm row ("❯    Submit"), where no
  // numbered option reads as selected. That state is reported rather than left
  // as "nothing selected", because a walk that starts there has to know it is
  // BELOW every option (review-caught 2026-09-03: the confirm-row fixture made
  // the delivery refuse with "cannot tell what the dialog has highlighted").
  const confirmSelected = region.some((line) => /^\s*❯\s+(submit|next)\s*$/i.test(line));
  ...
  for (let i = firstOptionLine; i <= regionEnd; i += 1) {
    ...
    // The confirm row belongs to the question, not to the option above it. It
    // is only ever the LAST line of the option block (a divider or the block's
    // end follows it), which keeps a hard-wrapped description whose
    // continuation line happens to read "Next" out of this.
    if (!optionByLine.has(i) && CONFIRM_ROW_RE.test(raw.trim())) {
      const next = i + 1 <= regionEnd ? (gutter ? lines[i + 1].slice(0, gutter) : lines[i + 1]) : '';
      if (i === regionEnd || !next.trim() || isDivider(next)) continue;
    }
  ...
  const options = groups.map((group) => {
    // A multi-select row carries its own state: "[ ]" unticked, "[✔]" ticked.
    ...
    // The free-text test runs on the row's TEXT, after the checkbox is off it.
    // Reading it before meant a multi-select question's free-text row was
    // "[ ] Type something", which the anchored test could never match, so that
    // question offered no way to type at all (2026-08-09).
  ...
  // Only a multi-select question draws per-row checkboxes, so one row carrying
  // one is proof, and it survives any re-wording of the footer.
  const multiSelect = footerSaysToggle || options.some((option) => option.checked !== undefined);

  const keys = {
    // WHAT THE CLI SAYS TODAY, not what it said when this was written
    // (2026-08-09). A batched AskUserQuestion's footer reads "Enter to select ·
    // Tab/Arrow keys to navigate · Esc to cancel"; it has not said "Tab to
    // switch questions" for some time. Matching only the old wording made this
    // false for every real batch, so the card rendered no way to reach question
    // 2 and Pat had none: "ability to switch to previous / next questions ...
    // it completely is busted". A SINGLE question says "↑/↓ to navigate"
    // instead and must stay false, because there is nothing to switch between.
    switchQuestions: /tab to switch questions/i.test(footerBlock)
      || /tab\s*\/\s*arrow keys[^\n]*navigat/i.test(footerBlock),
  ...
  return {
    ...
    clipped,
    confirmSelected,
    // The batch's own review screen, by its sentinel and not by its option
    // labels: a real question could offer "Submit answers" and "Cancel".
    submitReview: resumeDialog && lines.some((line) => SUBMIT_SENTINEL_RE.test(line)),
    // What the review says will be submitted, one "→ answer" line per answered
    // question in batch order (the CLI lists only the answered ones), so the
    // card can show the answers and not just the headers.
    reviewAnswers: resumeDialog
      ? lines.slice(questionTop, firstOptionLine)
