...
function applyOp(input, op, ctx = {}) {
  ...
  switch (type) {
    ...
    case 'task.appendAssignment': {
      const task = byId(op.taskId);
      if (!task) return fail('that task no longer exists');
      const sessionId = cleanText(op.sessionId, 200);
      if (!sessionId) return fail('an assignment needs a session id');
      const stamp = new Date(now).toISOString();
      const line = `Assigned to Claude session ${sessionId} at ${stamp}`;
      task.notes = cleanText([task.notes, line].filter(Boolean).join('\n'), MAX_NOTES);
      return { ok: true, doc, taskId: touch(task, now).id };
    }
    ...
    case 'task.duplicate': {
      // Deep-copy the task and its descendants. Same facts (title, notes, dates,
      // tags, star, My Day, list, parent); fresh ids; every node opens (a
      // duplicate is new work, not a finished one). The copy root sits
      // immediately after the original among siblings without renumbering the
      // whole sibling run the way task.reorder does.
      const task = byId(op.taskId);
      if (!task) return fail('that task no longer exists');
      const tree = [task, ...descendants(doc, task.id)];
      const idMap = new Map();
      for (const node of tree) idMap.set(node.id, idFactory('t'));
      ...
      for (const sib of siblings) {
        if (sib.id === task.id) continue;
        if (sib.order >= insertOrder) sib.order += 1;
      }

      // Assignment breadcrumbs name the ORIGINAL's session; a copy carrying
      // them would claim work that never touched it. Matches the line shape
      // appendAssignment writes (id, " at ", then ONE trailing token, which
      // also catches a stamp the notes cap truncated mid-ISO), so a hand-typed
      // sentence continuing past the stamp survives; and it returns a STRING
      // always, because notes is a string everywhere in this model and a null
      // here crashed the search selector (review round 2, 2026-08-25).
      const stripAssignments = (notes) => {
        if (!notes) return notes;
        return String(notes).split('\n')
          .filter((line) => !/^Assigned to Claude session \S+ at \S*$/.test(line))
          .join('\n').replace(/\n+$/, '');
      };
      const copies = [];
      for (const node of tree) {
        ...
        copies.push({
          ...
          depth: node.depth,
          title: node.title,
          notes: stripAssignments(node.notes),
          done: false,
          completedAt: null,
          completedBy: null,
          starred: node.starred,
          myDayDate: node.myDayDate,
          dueDate: node.dueDate,
          tags: [...node.tags],
          createdAt: now,
          updatedAt: now,
          order: isRoot ? insertOrder : node.order,
        });
      }
      doc.tasks.push(...copies);
      return { ok: true, doc, taskId: idMap.get(task.id) };
    }
