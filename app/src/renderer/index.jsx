...
import { WhiteboardView } from './whiteboard/WhiteboardView.jsx';
...
import { useDraftStore } from './use-draft-store.js';
import { appendTranscription } from './stage/command-bar-attachments.cjs';
...
const { emptyDraft } = draftStore;
import gridNav from './stage/grid-nav.cjs';
import unseenCompletions from './stage/unseen-completions.cjs';
import taskbarBadge from './stage/taskbar-badge.cjs';
...
const { planTaskbarBadge } = taskbarBadge;
const { createChain, chainTick, chainSendResult } = handoffChainLib;
const { isOrchestrationCwd } = sidebarModelShared;

// Exact Slate tokens from styles.css: --warn for blocked and --ac for finished.
const TASKBAR_BADGE_COLORS = { amber: '#d4a537', accent: '#437ffe' };

function renderTaskbarBadge(plan) {
  if (plan.kind === 'clear') return { dataUrl: null, description: plan.description };
  // The taskbar consumes a 16 CSS-pixel overlay. A 2x backing store keeps the
  // circle and type crisp at both 16 and 32 physical pixels.
  const logicalPx = 16;
  const scale = 2;
  const canvas = document.createElement('canvas');
  canvas.width = logicalPx * scale;
  canvas.height = logicalPx * scale;
  const context = canvas.getContext('2d');
  context.scale(scale, scale);
  context.fillStyle = TASKBAR_BADGE_COLORS[plan.kind];
  context.beginPath();
  context.arc(8, 8, 7.5, 0, Math.PI * 2);
  context.fill();
  context.fillStyle = '#ffffff';
  context.font = `700 ${plan.label.length > 1 ? 8 : 10}px "Schibsted Grotesk", sans-serif`;
  context.textAlign = 'center';
  context.textBaseline = 'middle';
  context.fillText(plan.label, 8, 8.25);
  return { dataUrl: canvas.toDataURL('image/png'), description: plan.description };
}

const TILE_STORE_KEY = 'harbor-slate-stage';
// Which sessions finished while Pat was away. See stage/unseen-completions.cjs
// for why this is a durable watermark rather than a watched transition.
const SEEN_STORE_KEY = 'harbor-seen';
...
const NEW_SESSION_DEFAULT_VERSION = 3;
// v2 (2026-07-27) was the opus ALIAS so a new flagship needed no Harbor
// change. v3 (2026-08-21) deliberately PINS claude-opus-4-8 on Pat's order:
// the alias now resolves to Opus 5, which he has rejected outright, so
// "current flagship" is exactly what the default must not mean. Un-pin only
// when he says so.
const FALLBACK_NEW_SESSION_DEFAULT = {
  provider: 'claude', model: 'claude-opus-4-8', effort: 'xhigh', v: NEW_SESSION_DEFAULT_VERSION,
};
const MAX_TILES = 16;
const VOICE_TOOL_NAMES = voiceToolsModule.TOOL_NAMES;

// Electron wraps a main-process throw as "Error invoking remote method 'x':
// Error: <msg>". The wrapper is noise to a human reading the status bar (Pat
// read one on 2026-08-12 and reasonably concluded Harbor was broken rather
// than refusing); keep only the message.
    ...
    <div className="daemon-banner" data-degraded="true" role="status">
      <span className="daemon-banner-text">
        {String(banner.error || '').startsWith('protocol_mismatch') || String(banner.error || '').startsWith('schema_version')
          ? `the session daemon changed underneath Harbor (${banner.error}); terminals are paused until it reconnects.`
...
function App() {
  ...
  // provisional session id -> the real UUID it upgraded to. A dictation started
  // in a brand-new session captures its provisional id; by the time Whisper
  // returns, the draft may have been renamed to the real id, so appendSessionDraft
  // resolves through this before writing, or the transcript lands on a dead id
  // (2026-09-03 round-2 fix).
  const draftAliasRef = useRef(new Map());
  ...
  // `tiles`, `selectedId` and `focusedId` are DERIVED from the persisted stage
  // further down (after sessionsById exists), never read raw: see resolveStage.
  ...
  // Questions that arrived through the hook lane (main/providers/ask-inbox.js):
  // the pending set, keyed by session, drives the form in the window and the
  // amber "needs your answer" cue in the rail. Nothing here reads a pty.
  const [asks, setAsks] = useState([]);
  useEffect(() => {
    if (!window.harbor.asks) return undefined;
    window.harbor.asks.list().then((list) => setAsks(Array.isArray(list) ? list : [])).catch(() => {});
    return window.harbor.asks.onChanged((list) => setAsks(Array.isArray(list) ? list : []));
  }, []);
  const asksBySession = useMemo(() => {
    const map = new Map();
    for (const ask of asks) if (ask?.sessionId && !ask.answered && !map.has(ask.sessionId)) map.set(ask.sessionId, ask);
    return map;
  }, [asks]);
  ...
  // ---- the stage the app operates on, resolved from the persisted store ----
  // A tile is resolvable when the model names its session, or when it is a
  // synthetic window the model cannot know yet (a pane-keyed provisional, a
  // just-launched session). Everything downstream (the grid, keyboard nav,
  // transcript subscriptions, the command bar target) sees only resolvable
  // tiles; the STORE keeps the rest, hidden, until their sessions return.
  // Live-caught 2026-09-04: the old cull-and-persist wiped fourteen open
  // windows over a model that was short for one refresh. Never delete a
  // persisted tile from a model snapshot again (stage-resolve.cjs).
  const isTileResolvable = useCallback((id) => (
    id.startsWith('pane:') || id.startsWith('live:') || launchMetaRef.current.has(id) || sessionsById.has(id)
  // launchFactsVersion is the signal that launchMetaRef changed.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ), [sessionsById, launchFactsVersion]);
  const isTileResolvableRef = useRef(isTileResolvable);
  isTileResolvableRef.current = isTileResolvable;
  const resolvedStage = useMemo(() => resolveStage({
    tiles: stage.tiles,
    selectedId: stage.selectedId,
    focusedId: stage.focusedId || null,
    isResolvable: isTileResolvable,
  }), [stage, isTileResolvable]);
  const { tiles, selectedId, focusedId } = resolvedStage;

  // ---- "finished while you were away" ----------------------------------
  // The store is seeded ONCE, on first run, so shipping this does not light up
  // every session already in the rail. Everything after that is a comparison
  // between the session's transcript clock and the last time Pat looked at it,
  // which is the only formulation that survives Harbor being closed.
  const [seenStore, setSeenStore] = useState(() => {
    try {
      return seedSeenStore(loadSeenStore(JSON.parse(localStorage.getItem(SEEN_STORE_KEY) || 'null')), Date.now());
    } catch {
      return seedSeenStore(loadSeenStore(null), Date.now());
    }
  });
  useEffect(() => {
    try { localStorage.setItem(SEEN_STORE_KEY, JSON.stringify(seenStore)); } catch { /* quota */ }
  }, [seenStore]);

  // Looking at a session IS checking it: the mark is refreshed on every update
  // while it stays selected, so a turn that finishes in the window Pat is
  // watching never gets a badge the moment he looks away.
  useEffect(() => {
    if (!selectedId) return;
    setSeenStore((prev) => markSeen(prev, selectedId, Date.now()));
  }, [selectedId, transcripts, sidebarModel]);
  ...
  // "Live outside Harbor": the transcript is hot but no daemon pane exists;
  ...
  const openSession = useCallback((session) => {
    ...
    setStagePersist((prev) => {
      ...
      if (tiles.length >= MAX_TILES) {
        // Make room: a hidden tile (its session absent from the model) goes
        // first, then the least-recently-selected window that is not the
        // active one. This is the ONLY place, besides an explicit close,
        // that removes a tile from the store.
  ...
  // There is deliberately NO effect here that removes tiles whose session is
  // missing from the model. That effect existed until 2026-09-04, "validated
  // once real data is in", and the first model to arrive is not always real
  // data: a boot whose history refresh failed, a get-state that rejected, an
  // index pass that skipped a project. Each of those shrank the model for one
  // update, the cull persisted the shrink, and Pat opened Harbor to "Nothing
  // on the stage" over fourteen live sessions. Absent tiles are now hidden by
  // resolveStage above and come back with their sessions.
  ...
  useEffect(() => {
    if (!sidebarModelLoaded) return;
    setStagePersist((prev) => {
      const plan = planProvisionalUpgrades({ ...prev, sessions: sessionsById.values() });
      if (!plan) return prev;
      for (const [from, to] of plan.renames) { renameDraft(from, to); draftAliasRef.current.set(from, to); }
      return { ...prev, tiles: plan.tiles, selectedId: plan.selectedId, focusedId: plan.focusedId };
    });
  }, [sidebarModelLoaded, sessionsById, renameDraft]);
  ...
  useEffect(() => {
    ...
    for (const id of [...openTranscriptsRef.current]) {
      if (!want.has(id)) {
        ...
        // A closed window's launch metadata and dictation alias are dead with
        // it; both Maps were only ever written, never deleted, so they grew for
        // the life of the process as sessions came and went (2026-09-06 leak
        // sweep). Release them here, on the same pass that forgets the transcript.
        launchMetaRef.current.delete(id);
        draftAliasRef.current.delete(id);
      }
    }
  ...
  // Heap decomposition cross-check (2026-09-06). The renderer OOMs with the DOM
  // flat, so the leak is JS objects; this measures how much of the heap the
  // transcripts state actually accounts for. If these bytes stay small while
  // usedHeapMB climbs to the ceiling, the leak is provably OUTSIDE the
  // transcripts Map (points at a per-update accumulator elsewhere); if they
  // track it, the Map is not as bounded as believed. Every 30s off a ref, so it
  // never runs on the ~2/sec update path.
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      let sessions = 0; let blocks = 0; let textBytes = 0; let imageBytes = 0; let imageBlocks = 0;
      for (const data of transcriptsRef.current.values()) {
        sessions += 1;
        for (const b of (data?.blocks || [])) {
          blocks += 1;
          if (b.text) textBytes += b.text.length;
          if (b.images?.length) { imageBlocks += 1; for (const img of b.images) imageBytes += (img.dataUri?.length || 0); }
        }
      }
      try {
        report({ kind: 'transcript-size', at: new Date().toISOString(), context: {
          sessions, blocks, imageBlocks,
          textMB: Math.round(textBytes / 1048576), imageMB: Math.round(imageBytes / 1048576),
        } });
  ...
  // sessionId -> 'finished' | 'blocked', for the rail rows and the window
  // headers. Working/blocked come from the OPEN transcript where there is one
  // and from the daemon's agent status otherwise: agent detection lags 60-150s,
  // and this feature is about promptness (same reason the voice settle-watcher
  // reads the transcript).
  const { attention, attentionDescriptors } = useMemo(() => {
    const map = new Map();
    const descriptors = [];
    // The taskbar badge only speaks for sessions OPEN AS WINDOWS (badgeCounts
    // skips descriptors without open); the rail pips keep the wider per-row
    // semantics. Tiles persist in localStorage, so this survives a relaunch.
    const openIds = new Set(tiles.map((tile) => tile.sessionId));
    for (const [id, session] of sessionsWithSynthetic) {
      const header = transcripts.get(id)?.header || null;
      const descriptor = {
        id,
        open: openIds.has(id),
        isHistorical: Boolean(session.isHistorical),
        activeMs: session.lastActiveMs,
        working: header ? Boolean(header.working) : session.agentStatus === 'working',
        // A question waiting in the hook lane is the strongest blocked signal
        // there is: Claude is inside the tool call until the form answers.
        blocked: asksBySession.has(session.id) || (header ? Boolean(header.blocked) : session.agentStatus === 'blocked'),
        isSelected: id === selectedId,
        // Orchestration workers and BATCH TITLE children finish all day; they
        // are not waiting for Pat, and counting them pinned the badge at 9+.
        excluded: Boolean(session.isChildTask)
          || isOrchestrationCwd({ cwd: session.cwd, label: session.project }),
      };
      descriptors.push(descriptor);
      const state = attentionFor(descriptor, seenStore);
      if (state) map.set(id, state);
    }
    return { attention: map, attentionDescriptors: descriptors };
  }, [sessionsWithSynthetic, transcripts, selectedId, seenStore, tiles, asksBySession]);

  // Taskbar status projects the same descriptors as the rail pips, with one
  // deliberate difference (badgeCounts): blocked is a LIVE state that ignores
  // selection and the seen store, because the badge exists for when Pat is
  // alt-tabbed away and an unanswered question needs him regardless of which
  // window happens to be selected. Coalesce bursts into one native set.
  useEffect(() => {
    const plan = planTaskbarBadge(badgeCounts(attentionDescriptors, seenStore));
    const timer = window.setTimeout(() => {
      window.harbor.badge?.set(renderTaskbarBadge(plan));
    }, 250);
    return () => window.clearTimeout(timer);
  }, [attentionDescriptors, seenStore]);

  // Diagnostic, same posture as __harborSidebarStats: a marker that silently
  // fails to appear is indistinguishable from "nothing is waiting", so the
  // inputs to that decision are inspectable from a drive.
  useEffect(() => {
    window.__harborAttention = {
      seededAtMs: seenStore.seededAtMs,
      sessions: sessionsWithSynthetic.size,
      marked: [...attention.entries()],
    };
  }, [attention, seenStore, sessionsWithSynthetic]);
  ...
  // from daemon agent status, because agent detection can lag and the
  ...
  // ---- one-click handoff chain -------------------------------------------
  // /handoff -> /compact -> /pickup driven into ONE session (Pat picked the
  // button shape over a pure command, 2026-08-29). The planner is pure
  // (stage/handoff-chain.cjs); this owns the clock and the sends. Sends go by
  // the chain's own session id, so switching windows mid-chain never
  // retargets it, and they call session.send directly rather than
  // dispatchToSelected so a routine mid-compaction refusal retries quietly
  // instead of painting an error line. /pickup carries no path on purpose:
  // the pickup command itself loads the newest handoff file.
  const [handoffChains, setHandoffChains] = useState(() => new Map());
  const chainWorldRef = useRef({ transcripts, sessions: sessionsWithSynthetic, chains: handoffChains });
  useEffect(() => {
    chainWorldRef.current = { transcripts, sessions: sessionsWithSynthetic, chains: handoffChains };
  }, [transcripts, sessionsWithSynthetic, handoffChains]);
  const chainSendsInFlightRef = useRef(new Set());

  const applyChainSendResult = useCallback((sessionId, result) => {
    chainSendsInFlightRef.current.delete(sessionId);
    setHandoffChains((prev) => {
      const chain = prev.get(sessionId);
      if (!chain) return prev;
      const now = Date.now();
      const advanced = chainSendResult(chain, { ...result, now });
      if (advanced === chain) return prev;
      const next = new Map(prev);
      next.set(sessionId, advanced.status === 'active' ? advanced : { ...advanced, endedAt: now });
      return next;
    });
  }, []);

  const chainSend = useCallback(async (sessionId, text) => {
    const session = chainWorldRef.current.sessions.get(sessionId);
    if (!session) return { ok: false, reason: 'the session is no longer open' };
    const result = await window.harbor.session.send({
      sessionId: session.id,
      text,
      images: [],
  ...
  useEffect(() => {
    if (!handoffChains.size) return undefined;
    const timer = setInterval(() => {
      const now = Date.now();
      const { transcripts: snapshot, chains } = chainWorldRef.current;
      const updates = new Map();
      const sends = [];
      for (const [id, chain] of chains) {
        if (chain.status !== 'active') {
          // Terminal chains linger on the status row, then sweep themselves.
          const ttl = chain.status === 'failed' ? 30_000 : 6_000;
          if (now - (chain.endedAt || 0) > ttl) updates.set(id, null);
          continue;
        }
        if (chainSendsInFlightRef.current.has(id)) continue;
        const data = snapshot.get(id);
        const header = data?.header || null;
        const out = chainTick(chain, {
          now,
          working: Boolean(header?.working),
          blocked: Boolean(header?.blocked),
          missing: !data,
        });
        if (out.state !== chain) {
          updates.set(id, out.state.status === 'active' ? out.state : { ...out.state, endedAt: now });
        }
        if (out.action?.type === 'send') {
          chainSendsInFlightRef.current.add(id);
          sends.push([id, out.action.text]);
        }
      }
      if (updates.size) {
        setHandoffChains((prev) => {
      ...
      for (const [id, text] of sends) {
        chainSend(id, text).then((result) => applyChainSendResult(id, result));
      }
    }, 1000);
    return () => clearInterval(timer);
  }, [handoffChains.size, chainSend, applyChainSendResult]);

  const startHandoffChain = useCallback((sessionId) => {
    if (!sessionId) return;
    setHandoffChains((prev) => {
      if (prev.get(sessionId)?.status === 'active') return prev;
      const next = new Map(prev);
      next.set(sessionId, createChain({ sessionId, now: Date.now() }));
      return next;
    });
  }, []);
  ...
  const takeoverSelected = useCallback(async (draft = '', images = []) => {
    ...
    const result = await window.harbor.session.takeover({ sessionId: session.id })
      .then(() => ({ ok: true }))
      .catch((error) => ({ ok: false, reason: ipcErrorMessage(error) }));
    if (!result.ok) {
      setSendStates((prev) => {
        const next = new Map(prev);
  ...
  const openNewSession = useCallback(async (input) => {
    ...
    if (!folder && !sessionId && selectedId) {
      ...
      // A foreign-era session's cwd (/home/you/... seen from Windows, or
      // C:\... seen from Linux) does not exist on this machine: inheriting it
      // is what bricked every new-session attempt on 2026-08-12 (CreateProcess
      // error 2 in the keeper). Fall through to the picker instead.
      const inheritable = selected && !selected.isWindowsEra;
      folder = inheritable ? (selected.cwd || null) : null;
      sessionId = (folder || !inheritable) ? null : selected?.id;
    }
    if (!folder && sessionId) {
    ...
    if (request.immediate) {
      ...
      return window.harbor.session.newInProject({
        ...
        const anchor = sessionId || selectedId;
        if (anchor) {
          setSendStates((prev) => {
            const next = new Map(prev);
            next.set(anchor, { sessionId: anchor, phase: 'error', detail: ipcErrorMessage(error) });
            return next;
          });
        }
        return null;
  ...
  // Append dictated text to a SPECIFIC session's draft, reading that session's
  // latest text at call time. A recording bound to session A must land in A even
  // after the rail switches to B, so this is keyed by id rather than by the
  // current selection (2026-09-03).
  const appendSessionDraft = useCallback((sessionId, transcript) => {
    if (!sessionId || !transcript) return;
    // Follow any provisional -> real id upgrade that happened while recording, so
    // the transcript lands on the live session and not a dead provisional id.
    let id = sessionId;
    const alias = draftAliasRef.current;
    const seen = new Set();
    while (alias.has(id) && !seen.has(id)) { seen.add(id); id = alias.get(id); }
    const current = getDraft(id);
    patchDraft(id, { text: appendTranscription(current.text, transcript) });
  }, [getDraft, patchDraft]);
          ...
          <div className="center-pane" aria-label="Workspace">
            {view === 'orch' ? (
              orchProject ? (
                ...
                <OrchLiveRuns projects={sidebarModel.projects} onPick={setOrchProject} />
              )
            ) : view === 'tasks' ? (
              <TasksView
                doc={tasks.doc}
              ...
              <Stage
                ...
                onHandoffChain={startHandoffChain}
                ...
                appendSessionDraft={appendSessionDraft}
                xterm={xterm}
...
// A popped-out board window (main loads index.html?window=board) renders ONLY the
// whiteboard, filling the OS window: no rail, no titlebar, no session plumbing.
// Board data is file-backed over IPC, so it needs none of the app shell.
const isBoardWindow = new URLSearchParams(window.location.search).get('window') === 'board';
