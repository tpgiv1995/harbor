...
export const Conversation = memo(function Conversation({
  ...
  // expandedGroups is keyed by group key (a monotonic block key). Once the
  // underlying blocks slide out of the bounded window those keys can never
  // recur, so the Set only ever grew (2026-09-06 leak sweep). Drop keys that no
  // longer name a live group; return prev unchanged when nothing was stale so
  // this never loops.
  useEffect(() => {
    setExpandedGroups((prev) => {
      if (prev.size === 0) return prev;
      const live = new Set(items.filter((it) => it.type === 'action-group').map((it) => it.groupKey));
      let stale = false;
      for (const key of prev) { if (!live.has(key)) { stale = true; break; } }
