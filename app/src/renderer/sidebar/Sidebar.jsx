...
export function Sidebar({ onOpenOrch, onOpenSession, onCloseSession, onNewSession, openSessionIds, selectedSessionId, attention, searchRef, view, onViewChange, orchEnabled = true, taskAlert = 0 }) {
  ...
  // Live sessions that could be windows and are not yet: the "open live" set.
  // Read from the UNFILTERED model so a rail narrowed to "today" or one
  // project still offers every running session; capped at the stage's own
  // window limit, newest activity first.
  const liveOpenable = useMemo(() => (model.projects || [])
    .flatMap((project) => project.sessions || [])
    .filter((session) => session.isLive && !session.isChildTask && !session.isWindowsEra && !openSessionIds?.has(session.id))
    .sort((a, b) => (b.lastActiveMs || 0) - (a.lastActiveMs || 0))
    .slice(0, 16), [model, openSessionIds]);
  ...
  return (
    <aside className="rail" aria-label="Session rail" style={{ '--rail-width': `${width}px` }} data-grouping={grouping} data-profile-count={profiles.length}>
      ...
      <div className="rail-head">
        ...
        {/* ONE CLICK BRINGS EVERY LIVE SESSION BACK TO THE STAGE (2026-09-04).
            The stage store was wiped that evening and fourteen live sessions
            had to be re-opened from the rail one by one; the daemon always
            knows which sessions are running, so the rail can offer them as a
            set. Workers and foreign-era rows stay out; already-open windows
            are left alone; the stage's own 16-window cap applies. */}
        {liveOpenable.length > 0 ? (
          <button
            type="button"
            className="rail-open-live"
            title={`Open every live session as a window (${liveOpenable.length} not open yet)`}
            aria-label={`Open all ${liveOpenable.length} live sessions`}
            onClick={() => { for (const session of liveOpenable) onOpenSession(session); }}
          >
            open live {liveOpenable.length}
          </button>
        ) : null}
        {profiles.length > 0 ? (
