...
//
// LINUX ONLY, AND IT MUST STAY THAT WAY UNTIL IT IS PORTED (2026-09-03). It
// launches through Playwright's electron helper and then calls setBounds({ x: 0,
// y: 0, width: 2560, height: 1600 }), which shows and restacks a full-screen
// window over whatever is on the desktop. Windows has no focus guard and Pat
// games fullscreen, so on that box this is the forbidden action, not a rough
// edge. The Windows equivalent is scripts/drive-live-voice-win.js: same drive,
// spawned with --no-focus-steal and parked off the visible desktop with
// SWP_NOACTIVATE before it touches anything.
//
// And the send stub below DOES NOT TAKE (measured 2026-09-03 on the real app):
// preload exposes the API as contextBridge.exposeInMainWorld('harbor',
// Object.freeze(api)), so assigning over window.harbor.session.send is a silent
// no-op and a live model's spoken instruction would reach the REAL send path.
// The check after it is therefore load-bearing: this drive must refuse to open a
// paid voice session it cannot keep away from Pat's work.
...
(async()=>{
  // Platform guard (2026-09-03): this drive restacks a full-screen window, which
  // is forbidden on Windows (no focus guard, Pat games fullscreen). Refuse to
  // even launch anywhere but Linux, BEFORE any window is created.
  if (process.platform!=='linux') {
    console.error('live-drive-voice.js is LINUX ONLY: it restacks a full-screen window. On Windows use scripts/drive-live-voice-win.js.');
    process.exit(1);
  }
  ...
  const stubbed = await page.evaluate(()=>{
    ...
    try {
      ...
      return window.harbor.session.send!==real;
    } catch { return false; }
  });
  // Fail CLOSED. A frozen contextBridge silently ignores the assignment above,
  // and a drive that believes it is sandboxed when it is not would let a live
  // model type a misheard instruction into one of Pat's real sessions.
  if (!stubbed) {
    // Throw so the finally below closes Electron; do not exit past it and leak
    // the window.
    throw new Error('REFUSING TO RUN: the session.send stub did not take (contextBridge freezes window.harbor), '
      + 'so a spoken instruction would reach the REAL send path. Fix the stub before running this.');
  }
  ...
  console.log(JSON.stringify(report,null,1));
    ...
    // Teardown ALWAYS runs, even on a throw between launch and here, so a failed
    // drive never leaves Electron and its fake mic alive (2026-09-03).
    try{await page?.evaluate(()=>window.harbor?.e2e?.quit?.());}catch{}
    if (app) { await app.close({force:true}).catch(()=>{}); try{app.process()?.kill('SIGKILL');}catch{} }
  }
  process.exit(0);
