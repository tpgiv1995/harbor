...
// THIS FILE IS A SIMULATION AND IT HAS BEEN WRONG BEFORE. On 2026-08-09 the
// batch scene below implemented Tab, and only Tab, for moving between
// questions; the real CLI takes ←/→ and Shift+Tab as well, its footer had
// changed from "Tab to switch questions" to "Tab/Arrow keys to navigate", and
// its "Type something" row turned out to be an inline text FIELD rather than a
// door to a composer. Building against this file would have reproduced all
// three mistakes. Before trusting any behaviour here, check it against the real
// captures in test/fixtures/askuserquestion/real-batch-*.txt and
// real-text-row-typed-120x60.txt, or drive a real dialog and take new ones.
...
function drawMulti() {
  ...
  // THE CONFIRM IS ITS OWN UNNUMBERED ROW, drawn under the last option. This
  // scene used to confirm on a bare Enter anywhere, which a real capture on
  // 2026-08-09 proved wrong: Enter on an option row TOGGLES it, and the answer
  // is committed on this row. `selected` one past the last option is it.
  rows.push(` ${selected === MULTI_OPTIONS.length + 1 ? '❯' : ' '}    Submit`);
...
function drawBatch() {
  ...
  process.stdout.write(`\x1b[2J\x1b[H${[
    ...
    '  Chat about this',
    '',
    'Enter to select · Tab/Arrow keys to navigate · n to add notes',
    'Esc to cancel',
  ].join('\r\n')}\r\n`);
}

// The multi scene has one row past its options: the unnumbered Submit.
const MAX_OPTION = SCENE === 'clipped' ? 5 : SCENE === 'multi' ? MULTI_OPTIONS.length + 1 : 2;
...
process.stdin.on('data', (chunk) => {
  ...
  for (;;) {
    ...
    // Question navigation. The real dialog takes → and Tab for next, ← and
    // Shift+Tab for previous (measured 2026-08-09); this scene used to
    // implement Tab alone, which is why a card that sent arrows looked broken
    // here while working against the CLI.
    if (SCENE === 'batch' && (buf.startsWith('\t') || buf.startsWith('\x1b[C'))) {
      questionIndex = (questionIndex + 1) % QUESTIONS.length;
      selected = 1;
      buf = buf.slice(buf.startsWith('\t') ? 1 : 3); draw(); continue;
    }
    if (SCENE === 'batch' && (buf.startsWith('\x1b[Z') || buf.startsWith('\x1b[D'))) {
    ...
    if (buf.startsWith('\r') || buf.startsWith('\n')) {
      ...
      if (SCENE === 'multi') {
        if (selected === MULTI_OPTIONS.length + 1) {
          finish(`PICKED:${[...multiChecked].sort((a, b) => a - b).join(',') || 'none'}`);
        }
        // On an option row Enter toggles, exactly as the real dialog does.
        if (multiChecked.has(selected)) multiChecked.delete(selected);
        else multiChecked.add(selected);
        buf = buf.slice(1); draw(); continue;
      }
