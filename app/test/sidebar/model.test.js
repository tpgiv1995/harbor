...
test('orchestration debris collapses into one project group without changing session identity', () => {
  const model = mergeSidebarModel({
    historySessions: [
      { id: 'root', lastActive: '2026-08-22 12:00', project: 'C--dev--orch-batch-7', cwd: 'C:\\dev\\.orch\\batch-7', title: 'Root batch' },
      { id: 'repo', lastActive: '2026-08-22 11:00', project: 'C--dev--orch-batch-fwscrub-widget-chatbot', cwd: 'C:\\dev\\.orch\\batch-fwscrub\\widget-chatbot', title: 'Repo batch' },
      { id: 'nested', lastActive: '2026-08-22 10:00', project: 'C--dev-misc-ad-hoc--orch-b1-foundation', cwd: 'C:\\dev\\misc-ad-hoc\\.orch\\b1-foundation', title: 'Nested batch' },
      { id: 'label-only', lastActive: '2026-08-22 09:00', project: 'C--dev-missing--orch-b4', cwd: null, title: 'Missing cwd' },
      // A cursor seat's worktree was deleted after the merge, so its cwd is
      // unrecoverable and its label is cursor's run-collapsed munge.
      { id: 'cursor-lane', lastActive: '2026-08-22 08:45', project: 'C-dev-orch-e1-widgetpicker', cwd: null, title: 'Cursor seat, worktree gone' },
      // The research workspace shape: orchestration scratch under the app's
      // own cache, no .orch segment anywhere in the path.
      { id: 'research', lastActive: '2026-08-22 08:40', project: 'harbor/orch-research-2026-01-01', cwd: 'C:\\Users\\user\\.cache\\harbor\\orch-research-2026-01-01', title: 'Research workspace' },
      { id: 'probe', lastActive: '2026-08-22 08:35', project: 'orch-research-2026-01-01/probe', cwd: 'C:\\Users\\user\\.cache\\harbor\\orch-research-2026-01-01\\probe', title: 'Probe session' },
      { id: 'cache-label', lastActive: '2026-08-22 08:30', project: 'C--Users-user--cache-harbor-orch-research-2026-01-01', cwd: null, title: 'Claude munge of the research workspace, cwd lost' },
      { id: 'harbor', lastActive: '2026-08-22 08:00', project: 'harbor', cwd: 'C:\\dev\\harbor', title: 'Harbor' },
      { id: 'normal-nested', lastActive: '2026-08-22 07:00', project: 'tools/widget', cwd: 'C:\\dev\\tools\\widget', title: 'Widget' },
      { id: 'near-miss', lastActive: '2026-08-22 06:00', project: 'C--dev-orchestrator-tools', cwd: null, title: 'orch prefix without the bare token' },
      { id: 'foreign', lastActive: '2026-08-22 05:00', project: 'linux: C--dev--orch-old', cwd: null, title: 'Foreign era' },
    ],
  });

  const orchestration = model.projects.find((project) => project.label === 'Orchestration');
  assert.ok(orchestration);
  assert.equal(orchestration.isOrchestration, true);
  assert.deepEqual(
    orchestration.sessions.map((session) => session.id),
    ['root', 'repo', 'nested', 'label-only', 'cursor-lane', 'research', 'probe', 'cache-label'],
  );
  assert.equal(orchestration.sessions.find((session) => session.id === 'nested').id, 'nested', 'a grouped session still resolves by its original id');
  assert.ok(flattenSidebarRows(model, { collapsedProjects: new Set() }).rows.some(
    (row) => row.kind === 'session' && row.session.id === 'nested',
  ), 'an expanded orchestration group exposes an openable session row');
  assert.ok(model.projects.some((project) => project.label === 'harbor'));
  assert.ok(model.projects.some((project) => project.label === 'tools/widget'));
  assert.ok(model.projects.some((project) => project.label === 'C--dev-orchestrator-tools'));
  assert.ok(model.projects.some((project) => project.label === 'linux: C--dev--orch-old' && project.isWindowsEra));
});

test('orchestration classification keys on the orch path token and stays platform-pure', () => {
  assert.equal(isOrchestrationCwd({ cwd: 'C:\\dev\\.orch\\batch-7' }), true);
  assert.equal(isOrchestrationCwd({ cwd: 'C:/dev/.orch/batch-fwscrub/widget-chatbot' }), true);
  assert.equal(isOrchestrationCwd({ cwd: '/home/pat/dev/misc/.orch/b1-foundation' }), true);
  // The research workspace: an orch- prefixed SEGMENT is orchestration even
  // with no .orch anywhere above it (live-caught 2026-08-23 as four ungrouped
  // rail groups; ~/.cache/harbor/orch-research-<date> and its probe subdir).
  assert.equal(isOrchestrationCwd({ cwd: 'C:\\Users\\user\\.cache\\harbor\\orch-research-2026-01-01' }), true);
  assert.equal(isOrchestrationCwd({ cwd: 'C:\\Users\\user\\.cache\\harbor\\orch-research-2026-01-01\\probe' }), true);
  // Munged labels, kept when a row loses its cwd. Claude munges every
  // character ('\.orch\' -> '--orch-'); cursor collapses runs ('\.orch\' ->
  // '-orch-'), so in munge space a dot-dir and a bare orch- segment read the
  // same. The dash-delimited token 'orch' is the rule for both spellings.
  assert.equal(isOrchestrationCwd({ label: 'C--dev-misc-ad-hoc--orch-b1-foundation' }), true);
  assert.equal(isOrchestrationCwd({ label: 'C-dev-orch-e1-widgetpicker' }), true);
  assert.equal(isOrchestrationCwd({ label: 'C--Users-user--cache-harbor-orch-research-2026-01-01' }), true);
  // A name that merely CONTAINS orch never shortens to the bare token.
  assert.equal(isOrchestrationCwd({ label: 'C--dev-orchestrator-tools' }), false);
  assert.equal(isOrchestrationCwd({ label: 'C-dev-pytorch-lab' }), false);
  // A real cwd keeps the distinction the munges lose: a project honestly
  // named with an interior '-orch-' fragment is not orchestration scratch.
  assert.equal(isOrchestrationCwd({ cwd: 'C:\\dev\\smart-orch-tools' }), false);
  assert.equal(isOrchestrationCwd({ cwd: 'C:\\dev\\harbor' }), false);
  assert.equal(isOrchestrationCwd({ cwd: 'C:\\dev\\tools\\widget' }), false);
  assert.equal(isOrchestrationCwd({ label: 'win: C--dev--orch-old' }), false);
  assert.equal(isOrchestrationCwd({ label: 'linux: C--dev--orch-old' }), false);
});

test('the Orchestration project is included in boot-time default collapse', () => {
  assert.match(sidebarSource, /project\.isOrchestration[\s\S]{0,120}!project\.hasLive/);
});
...
test('era sessions are flagged from either direction of the migration', () => {
  assert.equal(isWindowsEra('win: example-app'), true);
  // The Linux install's history, seen from the Windows host (2026-08-12): the
  // flag means "foreign era", and only its name predates the second migration.
  assert.equal(isWindowsEra('linux: dev/example-app'), true);
