...
const { AI_BIN } = require('../../src/main/actions/launch.js');
...
test('authenticated new-session launches a candidate folder with the selected provider argv, while anonymous is refused', async (t) => {
  ...
  // Homes are provider-scoped since the provider-keeper work: a CLAUDE config
  // home is never forwarded to a codex launch (bin/ai would have exported it
  // as the child's CODEX_HOME, pointing codex at a directory that is not a
  // codex home). With no codex profile configured for this account, the argv
  // carries no --home and codex uses its own default home.
  const logicalArgv = [
    '--provider', 'codex', '--model', 'gpt-5.6-sol', '--effort', 'xhigh',
  ];
  assert.deepEqual(allowed.result.argv, logicalArgv);
  assert.equal(allowed.result.cwd, h.recent);
  assert.deepEqual(h.launches[0].argv, scriptInvocation(AI_BIN, logicalArgv).args);
