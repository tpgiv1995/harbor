...
test.describe('Harbor Slate E2E', () => {
  ...
  test('2) search narrows the rail; Ctrl+K focuses it; filters apply', async () => {
    ...
    try {
      ...
      // The rail-head count shows the narrowed fraction, so assert the count
      // drops below the indexer total rather than comparing raw row counts.
      //
      // A time filter no longer auto-expands the matching projects (2026-08-08).
      // It used to, and that is exactly what made the caret dead: the resting
      // filter IS a time filter, so "a chip overrides collapse" meant "collapse
      // never applies". An explicit collapse now survives every chip and only a
      // text SEARCH flattens it (collapse-policy.cjs, spec 2c). That makes this
      // assertion independent of expansion state, which is why it still holds.
  ...
  test('4) two windows: selection raises, Ctrl+1/2 switches, command bar retargets', async () => {
    ...
    try {
      ...
      await page.locator(`.win2[data-session-id="${opened[1]}"] .wh .ti`).click();
      await expect(page.locator(`.win2[data-session-id="${opened[1]}"]`)).toHaveClass(/sel/);
  ...
  test("21) a window's + starts a sibling session in that window's own project and plan, with no picker and no popover", async () => {
    ...
    try {
      ...
      await page.locator(`.win2[data-session-id="${pair[0].id}"] .wh .ti`).click();
  ...
  test('2c) the caret collapses a project in the DEFAULT view, not only under All', async () => {
    const { electronApp, page } = await launch();
    try {
      // Pat, 2026-08-08: "projects are not collapsing in side panel". The rail's
      // resting filter is the 48h chip, and the old rule ignored collapse
      // whenever ANY time filter was active, so the caret was dead in the only
      // view anyone actually sits in: it could not hide a session and it could
      // not even flip its own arrow. It "worked" only after clicking All, which
      // is why it survived every existing spec. This drives the real click in
      // the DEFAULT state and requires the sessions to actually leave the rail.
      //
      // The count comes from `.rail-count`, not from counting `.sr-wrap` nodes:
      // the rail is VIRTUALIZED, so collapsing one project scrolls another's
      // rows into the DOM and a node count can rise while the model shrinks.
      // The head count is the flattener's own sessionRowCount.
      const railSessionCount = async () => {
        const text = await page.locator('.rail-count').innerText();
      ...
      const caret = wrap.locator('.sidebar-caret');

      // Boot state is not assumed: non-live projects auto-collapse on first
      // model (a behaviour this same bug had also silently defeated), so settle
      // on EXPANDED first and assert from a known start.
      if ((await caret.textContent())?.trim() === '▸') {
        await caret.click();
        await page.waitForTimeout(400);
      }
      expect((await caret.textContent())?.trim(), 'project starts expanded').toBe('▾');

      const expanded = await railSessionCount();
      expect(Number.isFinite(expanded)).toBe(true);
      expect(expanded).toBeGreaterThan(0);

      // The click under test: the caret itself, which is what Pat clicks.
      await caret.click();
      await page.waitForTimeout(400);

      expect((await caret.textContent())?.trim(), 'the caret must flip').toBe('▸');
      const collapsed = await railSessionCount();
      expect(collapsed, 'collapsing must remove that project\'s sessions').toBeLessThan(expanded);

      // And back, so the assertion cannot pass by the rail merely going empty.
      await caret.click();
      await page.waitForTimeout(400);
      expect((await caret.textContent())?.trim()).toBe('▾');
      expect(await railSessionCount(), 'expanding restores them').toBe(expanded);
    } finally {
      await closeHarbor(electronApp, page);
    }
  });
  ...
  test('the Artifacts view lists agent-produced files and renders HTML without navigating the app', async () => {
    ...
    try {
      ...
      // Notes adds the fifth top-level view.
      await expect(page.locator('.view-switch-btn')).toHaveCount(6);
  ...
  test('6) live command-bar send delivers real bytes into an isolated herdr pane', async () => {
    ...
    try {
      ...
      // The window now reads live; the bar is armed. Liveness is the single
      // run-state chip now (the separate "live"/"active" chips were folded in,
      // 2026-08-31).
      await expect(page.locator(`.win2[data-session-id="${meta.id}"] .runstate`).first()).toBeVisible();
      await expect(page.locator('.resume-chip')).toHaveCount(0);
  ...
  test('6e) a BATCHED AskUserQuestion renders its tab strip, labels and descriptions, and answers every question', async () => {
    ...
    try {
      ...
      await verifyClient.sendKeys(simPaneId, ['enter']);
      await expect.poll(paneText, { timeout: 10000 }).toContain('Tab/Arrow keys to navigate');
      ...
      // Both directions move between questions, through the real pty. The card
      // used to offer only a forward "Next question ⇥": a batch you can walk
      // one way is a batch you cannot correct, and the dialog's own strip draws
      // a ← and a → (2026-08-09).
      await card.locator('.tile-ask-key', { hasText: 'Next →' }).click();
      await expect(card.locator('.tile-ask-q'))
        .toHaveText('Which totals should a custom range show?', { timeout: 10000 });
      await card.locator('.tile-ask-key', { hasText: '← Previous' }).click();
      await expect(card.locator('.tile-ask-q'))
        .toHaveText('How should the custom range appear in the filter?', { timeout: 10000 });
      // And the keyboard does the same thing, which is what was actually asked
      // for: focus the card and press the arrow keys.
  ...
  test('6g) a multi-select question is ticked and submitted from its own window', async () => {
    ...
    try {
      ...
      // The confirm row is the dialog's own, drawn under the options.
      await expect.poll(paneText, { timeout: 8000 }).toContain('Submit');
  ...
  // Pat, 2026-08-14: with 12-14 windows open and a few actually running, he
  // closes Harbor, comes back later, and cannot tell which sessions finished
  // and are waiting for him. Backdating the watermark reproduces exactly that
  // state, because "was away" and "watermark is old" are the same thing to the
  // rule.
  test('sessions that finished while you were away are marked, and checking one clears it', async () => {
    const { electronApp, page } = await launch();
    try {
      await page.waitForSelector('.rail', { timeout: 30000 });
      // A watermark from the epoch: every session in the corpus has written
      // since, so each counts as finished-and-unchecked.
      await page.evaluate(() => {
        localStorage.setItem('harbor-seen', JSON.stringify({ version: 1, seededAtMs: 1, seen: {} }));
      });
      await page.reload();
      await page.waitForSelector('.rail', { timeout: 30000 });
      await page.waitForFunction(() => {
      ...
      // A COLLAPSED project renders no session rows at all, so the rollup count
      // on the group header is the only thing visible until it is expanded.
      // That is the whole reason the rollup exists (found by driving it: the
      // attention map was right and the rail still looked empty).
      await expect(page.locator('.pg-attn').first()).toBeVisible({ timeout: 20000 });
      ...
      await screenshot(page, 'attention-rail-marked.png');
      ...
      await screenshot(page, 'attention-cleared.png');

      // The mark is durable, not a render-time accident: it survived into
      // localStorage keyed by that session.
      const seen = await page.evaluate((id) => {
        const store = JSON.parse(localStorage.getItem('harbor-seen') || 'null');
        return store && store.seen ? store.seen[id] : null;
      }, markedId);
  ...
  test('command bar pastes an image as an attachment chip and grows for a multiline send', async () => {
    ...
    try {
      ...
      await expect.poll(async () => composerText(page)).toBe('');
      // TWO controls per chip since 2026-08-13: the thumbnail opens a preview,
      // the '×' removes. Until then the chip's only control was the '×', so the
      // only thing a click on an attachment could do was destroy it (Pat:
      // "clicking on it does nothing (unless you click the 'x')").
      expect(await page.locator('.image-chip .image-chip-open').count()).toBe(1);
      expect(await page.locator('.image-chip .image-chip-remove').count()).toBe(1);

      // Clicking the thumbnail opens the image, and Escape closes it. The
      // attachment SURVIVES being looked at: a preview must never consume the
      // thing it previewed.
      await page.locator('.image-chip .image-chip-open').first().click();
      await page.waitForSelector('.image-preview .image-preview-panel', { timeout: 5000 });
      await screenshot(page, 'attachment-preview.png');
      await page.keyboard.press('Escape');
      await expect(page.locator('.image-preview')).toHaveCount(0);
      expect(await page.locator('.image-chip').count()).toBe(1);

      // Remove clears it, and closes nothing else along the way.
      await page.locator('.image-chip .image-chip-remove').first().click();
