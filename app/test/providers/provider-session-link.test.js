...
test('the cursor project-dir munge collapses runs, matching the dirs on disk', () => {
  // 'C:\' and '\.' are RUNS of junk characters and cursor spends one dash on
  // each; the per-character spelling ('C--dev--orch-...') this copy carried
  // until 2026-08-23 pointed cursorSessionsFor at directories that never
  // exist on a Windows machine.
  assert.equal(cursorProjectDir('C:\\dev\\.orch\\e1-lane'), 'C-dev-orch-e1-lane');
  // A dotted username is the point of this fixture, not whose it is.
  assert.equal(cursorProjectDir('/home/ada.lovelace/dev/widget'), 'home-ada-lovelace-dev-widget');
});
...
test('keeper annotation must acknowledge before a link publishes, and transient failure retries', async () => {
  const home = makeHome();
  let attempts = 0;
  const remembered = [];
  const linker = createProviderSessionLinker({
    home,
    listPanes: () => [{
      pane_id: 'keeper-4', workspace_id: 'keeper-4', agent: 'codex', agent_session: null, cwd: '/work',
    }],
    paneAgentProcess: async () => ({
      pid: 99, startedMs: T0, cwd: '/work', argv: ['codex', 'resume', ID_B],
    }),
    annotateSessionIdentity: async (_paneId, identity) => {
      attempts += 1;
      assert.deepEqual(identity, { agent: 'codex', agentSession: ID_B });
      if (attempts === 1) throw new Error('daemon reconnecting');
      return { agent: 'codex', agent_session: ID_B };
    },
    rememberLink: async (fact) => remembered.push(fact),
  });
  assert.equal(await linker.resolveNow(), false, 'failed annotation does not publish an in-memory truth');
  assert.equal(linker.get('keeper-4'), null);
  assert.equal(await linker.resolveNow(), true, 'the next pass retries the same evidence');
  assert.equal(linker.get('keeper-4'), ID_B);
  assert.equal(attempts, 2);
  assert.equal(remembered.length, 1);
  assert.equal(remembered[0].cwd, '/work');
  linker.close();
});
