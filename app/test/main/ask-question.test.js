...
// ---- 2026-09-03: the second signal, the answered marks, and the review ----

const { matchQuestionText, currentQuestionIndex, answeredFromStrip, isReviewMenu } = require('../../src/main/ask-question.js');

test('the question line is a second signal when no option row matched', () => {
  const questions = [
    { question: 'How do you want me to produce the complete BIH file for the full roster?', header: 'Finish approach', options: [{ label: 'Reassemble + one BIH map' }] },
    { question: 'How should I hand off the finished file? (pick any)', header: 'Delivery', options: [{ label: 'Just the file' }] },
  ];
  assert.equal(matchQuestionText(questions, 'How should I hand off the finished file?'), 1, 'a prefix of the real question');
  assert.equal(matchQuestionText(questions, 'ow do you want me to produce the complete BIH file for the full roster?'), 0, 'a first character eaten by the pane');
  assert.equal(matchQuestionText(questions, 'How'), -1, 'too short to mean anything');
  assert.equal(matchQuestionText(questions, 'Do you want to proceed?'), -1, 'a different question is no match');
  // Only the dialog's own extra rows are visible: options cannot decide, the line can.
  const menu = { question: 'How should I hand off the finished file? (pick any)', options: [{ index: 4, label: 'Type something', isText: true }, { index: 5, label: 'Chat about this' }] };
  assert.equal(currentQuestionIndex(questions, menu), 1);
});

test('mergeAsk falls back to the question line and says which signal it used', () => {
  const ask = [
    { question: 'Where should the PDFs be generated?', header: 'PDFs', options: [{ label: 'In CI', description: 'ci' }, { label: 'Locally', description: 'local' }] },
    { question: 'How should the report be delivered?', header: 'Delivery', options: [{ label: 'Email' }, { label: 'Dropbox' }] },
  ];
  const menu = {
    question: 'How should the report be delivered?',
    options: [{ index: 3, label: 'Type something.', isText: true, selected: true }, { index: 4, label: 'Chat about this', selected: false }],
    clipped: true,
    keys: {},
  };
  const merged = mergeAsk(menu, ask);
  assert.equal(merged.matchedBy, 'question');
  assert.equal(merged.batch.currentIndex, 1);
  assert.equal(merged.header, 'Delivery');
  assert.deepEqual(merged.asked.map((q) => q.header), ['PDFs', 'Delivery']);
});

test('answered marks come from the strip, by header, and are unknown without one', () => {
  const tabs = [{ label: 'Finish app…', done: true, submit: false }, { label: 'Delivery', done: false, submit: false }, { label: 'Submit', done: false, submit: true }];
  assert.equal(answeredFromStrip(tabs, 'Finish approach'), true, 'a truncated tab label still names its header');
  assert.equal(answeredFromStrip(tabs, 'Delivery'), false);
  assert.equal(answeredFromStrip(tabs, 'Nowhere'), null);
  assert.equal(answeredFromStrip(null, 'Delivery'), null);
  assert.equal(answeredFromStrip(tabs, ''), null);
});

test('the review screen is labelled and carries the strip\'s answered marks, options untouched', () => {
  const menu = {
    question: 'Review your answers',
    options: [{ index: 1, label: 'Submit answers', selected: true }, { index: 2, label: 'Cancel', selected: false }],
    tabs: [{ label: 'Alpha', done: true, submit: false }, { label: 'Beta', done: false, submit: false }, { label: 'Submit', done: false, submit: true }],
    keys: {},
    submitReview: true,
  };
  const ask = [{ question: 'Alpha?', header: 'Alpha', options: [{ label: 'a' }] }, { question: 'Beta?', header: 'Beta', options: [{ label: 'b' }] }];
  assert.equal(isReviewMenu(menu), true);
  const merged = mergeAsk(menu, ask);
  assert.equal(merged.review, true);
  assert.deepEqual(merged.options.map((o) => o.label), ['Submit answers', 'Cancel']);
  assert.deepEqual(merged.batch.headers.map((h) => [h.header, h.answered]), [['Alpha', true], ['Beta', false]]);
  assert.equal(isReviewMenu({ options: [{ label: 'Yes' }, { label: 'No' }] }), false);
  assert.equal(
    isReviewMenu({ footer: 'Enter to select', options: [{ label: 'Submit answers' }, { label: 'Cancel' }] }),
    false,
    'a footered question that happens to offer those two labels is a question',
  );
});

test('answered marks prefer the tab at the question\'s own position, so shared prefixes cannot collide', () => {
  const tabs = [{ label: 'Delivery', done: true, submit: false }, { label: 'Delivery met…', done: false, submit: false }, { label: 'Submit', done: false, submit: true }];
  assert.equal(answeredFromStrip(tabs, 'Delivery', 0), true);
  assert.equal(answeredFromStrip(tabs, 'Delivery method', 1), false);
  assert.equal(answeredFromStrip(tabs, 'Delivery', -1), true, 'without a position the exact label wins');
  assert.equal(answeredFromStrip(tabs, 'Delivery method', -1), null, 'and an ambiguous prefix claims nothing');
});
