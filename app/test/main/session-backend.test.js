...
test('the selector reaches sessiond by default and refuses the retired backend', () => {
  const deps = {
  ...
  assert.throws(
    () => createControlClient({ env: { HARBOR_SESSION_BACKEND: 'herdr' }, ...deps }),
    /Herdr backend was retired/,
  );
});
...
test('the byte bridge follows the same selector and default', () => {
  ...
  assert.throws(
    () => createStreamSupervisor({ env: { HARBOR_SESSION_BACKEND: 'herdr' }, ...deps }),
    /Herdr backend was retired/,
  );
});
...
test('sessiond selection refuses before connecting when the isolation policy blocks it', () => {
  ...
  assert.throws(() => resolveSessionBackend({ HARBOR_SESSION_BACKEND: 'herdr' }), /Herdr backend was retired/);
...
test('the daemon adapter preserves the existing spawn, input, read, resize, process, and close drive', async () => {
  ...
  // Shaped for closePaneTab and signalPane, which read shell_pid and
