...
// This file drives REAL ConPTY panes, and pane teardown is the slow half: it
// cleared 10s twice on this machine under full-suite load and again on the
// hosted 2-core windows-latest runner (RPC "process request timed out after
// 10000ms", first CI flip run, 2026-08-14). Every wait here asserts that a
// thing HAPPENS, never that it is quick, so the file-wide deadline is sized
// for the slowest machine that legitimately runs it. A genuine hang still
// fails, just later.
// Overridable because the runner's serial pty pass legitimately begins inside
// the parallel pass's ConPTY teardown storm and sets a larger figure there.
const SLOW_PTY_MS = Number(process.env.HARBOR_TEST_SLOW_PTY_MS) > 0
  ? Number(process.env.HARBOR_TEST_SLOW_PTY_MS)
  : 30000;
if (!process.env.HARBOR_SESSIOND_REQUEST_TIMEOUT_MS) {
  process.env.HARBOR_SESSIOND_REQUEST_TIMEOUT_MS = String(SLOW_PTY_MS);
}
...
async function waitUntil(probe, message, timeoutMs = SLOW_PTY_MS) {
...
test('control adapter drives snapshot, spawn, input, screen, process, and close against a real sessiond pty', async () => {
  ...
  await client.sendInput(created.pane_id, { bytes: Buffer.from(line(IS_WIN32 ? 'echo CONTROL_BYTES_REACHED_PTY' : "printf 'CONTROL_BYTES_REACHED_PTY\\n'")) });
...
test('spawn persists declared identity, derives known agents, and leaves a plain shell unidentified', async () => {
  ...
  const suffix = IS_WIN32 ? '.exe' : '';
  const fakeClaude = path.join(harness.dir, `claude${suffix}`);
  const fakeCodex = path.join(harness.dir, `codex${suffix}`);
  const fakeCursor = path.join(harness.dir, `cursor-agent${suffix}`);
  for (const executable of [fakeClaude, fakeCodex, fakeCursor]) {
    if (IS_WIN32) fs.copyFileSync(SHELL, executable);
    else fs.writeFileSync(executable, '#!/bin/sh\nexec /bin/bash --noprofile --norc\n', { mode: 0o700 });
  }

  const fakeArgs = (extra = []) => IS_WIN32 ? ['/Q', '/K', 'rem', ...extra] : extra;

  const declared = await client.createWorkspace({
    argv: [fakeClaude, ...fakeArgs(['--session-id', ignoredDerivedSession])],
  ...
  const derivedClaude = await client.createWorkspace({
    argv: [fakeClaude, ...fakeArgs(['--session-id', derivedClaudeSession, '--noprofile', '--norc'])],
    cwd: harness.dir, env: {}, cols: 80, rows: 30,
  });
  const derivedCodex = await client.createWorkspace({
    argv: [fakeCodex, ...fakeArgs(['--noprofile', '--norc'])], cwd: harness.dir, env: {}, cols: 80, rows: 30,
  });
  const derivedCursor = await client.createWorkspace({
    argv: [fakeCursor, ...fakeArgs(['--noprofile', '--norc'])], cwd: harness.dir, env: {}, cols: 80, rows: 30,
  });
  const shell = await client.createWorkspace({
    argv: [SHELL, ...SHELL_ARGS], cwd: harness.dir, env: {}, cols: 80, rows: 30,
  });
...
test('byte bridge observes real frames, sends bytes, resizes, sizes dialogs, and fully detaches', async () => {
  ...
  streams.sendInput(paneId, { bytes: Buffer.from(line(IS_WIN32 ? 'echo STREAM_BYTES_REACHED_PTY' : "printf 'STREAM_BYTES_REACHED_PTY\\n'")) });
  ...
  streams.sendInput(paneId, line(IS_WIN32 ? 'echo STREAM_RESIZE_VISIBLE' : "stty size; printf 'STREAM_RESIZE_VISIBLE\\n'"));
  const resized = await waitUntil(async () => {
    const value = await control.readPane(paneId, { lines: 200 });
    const visible = value.read.text.includes('STREAM_RESIZE_VISIBLE');
    const sized = IS_WIN32 || /37 91/.test(value.read.text);
    return visible && sized ? value : null;
  ...
  streams.sendInput(paneId, line(IS_WIN32 ? 'echo DIALOG_SIZE_VISIBLE' : "stty size; printf 'DIALOG_SIZE_VISIBLE\\n'"));
  await waitUntil(async () => {
    const value = await control.readPane(paneId, { lines: 200 });
    return value.read.text.includes('DIALOG_SIZE_VISIBLE') && (IS_WIN32 || /57 123/.test(value.read.text));
  ...
  await waitUntil(async () => (await control.snapshot()).snapshot.panes.length === 0,
    'test cleanup closePane did not remove the real pty');
...
test('processInfo reports a pane process the way its consumers read it, deepest child last', async () => {
  ...
  const childCommand = IS_WIN32 ? 'powershell.exe -NoProfile -Command "Start-Sleep -Seconds 47"' : 'sleep 47';
  await client.sendText(created.pane_id, line(childCommand));
  const deepest = await waitUntil(
    async () => {
      const info = (await client.processInfo(created.pane_id)).process_info;
      const procs = info?.foreground_processes || [];
      const last = procs[procs.length - 1];
      return last?.argv?.join(' ').toLowerCase().includes(IS_WIN32 ? 'start-sleep' : 'sleep 47') ? last : null;
    },
...
test('the real send guards refuse a real sessiond pane sitting at a shell prompt', async () => {
  ...
  const created = await client.createWorkspace({
    argv: [SHELL, ...SHELL_ARGS],
    cwd: harness.dir,
    env: shellEnv(IS_WIN32 ? 'harbor-test$$ ' : 'harbor-test$ '),
    cols: 120,
    rows: 60,
  });
  await waitUntil(
