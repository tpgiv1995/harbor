...
test('channel metadata covers exactly the captured 110 methods and 21 pushes', () => {
  ...
  assert.equal(new Set(METHOD_CHANNELS.map(({ method }) => method)).size, 110);
  assert.equal(new Set(PUSH_CHANNELS).size, 22);
  ...
  assert.deepEqual(
    ...
    { 'local-only': 34, 'remote-safe': 37, mutating: 39 },
  );
  assert.deepEqual(
