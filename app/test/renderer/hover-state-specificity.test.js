...
test('the known instances each carry a state-aware hover', () => {
  const css = fs.readFileSync(CSS_PATH, 'utf8');
  for (const sel of [
    '.compose-mic.recording:hover',
    // The answer sheet's current step and picked row (2026-09-03; they
    // replaced the old card's .tile-ask-key.primary, the original instance).
    '.ask-step.cur:hover',
    '.ask-row.picked:hover',
