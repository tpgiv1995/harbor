...
function ownerFixture(overrides = {}) {
  ...
  const readFile = async (file) => {
    assert.equal(file, path.join('/fake/context', `${SESSION_ID}.json`));
...
test('a wedged /proc answers the probe with a refusal, not a hang', async () => {
  ...
  const probe = createSessionOwnerProbe({
    ...
    // cmdline does. The platform's own deadline is deliberately huge: an
    // explicit procDeadlineMs must outrank it, or a harness could never
    // shrink the wait.
    platform: { processInfo: () => new Promise(() => {}), processQuestionDeadlineMs: 9999 },
    scanForSessionOwner: () => new Promise(() => {}),
    procDeadlineMs: 50,
  });
  try {
    const started = Date.now();
    await assert.rejects(() => probe(id), /the process table may be wedged/);
...
// The wedge deadline above was sized for /proc and no caller ever resized it,
// so on win32 every process question died at 3000ms while the honest CIM
// answer takes ~3.2s (single pid) to ~4.8s (full table), measured idle on the
// real machine: three real takeover attempts on 2026-08-21 each refused at
// exactly +3.0s (send-log 05:21:51/05:22:17/05:22:27) and adopt-on-send was
// structurally dead on Windows. The platform now names its own honest
// deadline and the probe reads it when no explicit one is passed.
test('the probe deadline comes from the platform when the platform names one', async () => {
  const { createSessionOwnerProbe } = require('../../src/main/actions/takeover.js');
  const fs = require('node:fs');
  const fsp = require('node:fs/promises');
  const os = require('node:os');
  const path = require('node:path');
  const id = `platform-deadline-${process.pid}`;
  const contextDir = fs.mkdtempSync(path.join(os.tmpdir(), 'harbor-platform-deadline-'));
  fs.writeFileSync(
    path.join(contextDir, `${id}.json`),
    JSON.stringify({ session_id: id, pid: 424243 }),
  );
  const probe = createSessionOwnerProbe({
    contextDir,
    readFile: fsp.readFile,
    statFile: (target) => fsp.stat(target),
    platform: { processInfo: () => new Promise(() => {}), processQuestionDeadlineMs: 60 },
    scanForSessionOwner: () => new Promise(() => {}),
  });
  try {
    const started = Date.now();
    await assert.rejects(() => probe(id), /did not answer within 60ms \(the process table may be wedged\)/);
    assert.ok(Date.now() - started < 2000, 'the refusal must arrive at the platform deadline, not the /proc default');
  } finally {
    fs.rmSync(contextDir, { recursive: true, force: true });
  }
});
