...
test('11) duplicate copies facts into a fresh open task and prints the new id', () => {
  const { file } = sandbox();
  const added = json(file, ['add', 'Probe task', '--due', 'tomorrow', '--tag', 'x', '--notes', 'keep me']);
  json(file, ['done', 'Probe task']);

  const dup = json(file, ['duplicate', added.task.id]);
  assert.equal(dup.ok, true);
  assert.ok(dup.task.id);
  assert.notEqual(dup.task.id, added.task.id);
  assert.equal(dup.task.title, 'Probe task');
  assert.equal(dup.task.done, false);
  assert.equal(dup.task.dueDate, added.task.dueDate);
  assert.deepEqual(dup.task.tags, ['x']);
  assert.equal(dup.task.notes, 'keep me');

  // View "all" hides completed by design; --list --all is the surface that
  // shows both halves of a duplicate-of-done pair.
  const listed = json(file, ['list', '--list', 'Tasks', '--all']);
  const titles = listed.tasks.filter((t) => t.title === 'Probe task');
  assert.equal(titles.length, 2);
  const open = titles.filter((t) => !t.done);
  const closed = titles.filter((t) => t.done);
  assert.equal(open.length, 1);
  assert.equal(closed.length, 1);
  assert.equal(open[0].id, dup.task.id);
  assert.equal(closed[0].id, added.task.id);

  // Title fragment still works when it is unique.
  const unique = json(file, ['add', 'Only this title']);
  const byTitle = json(file, ['duplicate', 'Only this']);
  assert.notEqual(byTitle.task.id, unique.task.id);
  assert.equal(byTitle.task.title, 'Only this title');
  assert.equal(byTitle.task.done, false);
});

test('11b) duplicate refuses an ambiguous title the way done does', () => {
  const { file } = sandbox();
  json(file, ['add', 'Review the renewal']);
  json(file, ['add', 'Review the census']);
  const ambiguous = run(file, ['duplicate', 'Review the']);
  assert.equal(ambiguous.code, 1);
  assert.match(ambiguous.err, /matches 2 tasks/);
});
