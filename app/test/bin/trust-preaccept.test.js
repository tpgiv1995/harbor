...
const { claudeConfigFile, preacceptFolderTrust, trustKeysFor } = require('../../../bin/harbor-bin.cjs');
...
test('an already trusted folder is not rewritten', (t) => {
  ...
  fs.mkdirSync(configHome, { recursive: true });
  const file = path.join(configHome, '.claude.json');
  // "Already trusted" means EVERY key spelling the CLI might read is trusted:
  // on Windows that is the backslash key and the forward-slash key (see the
  // trustKeysFor specs below), elsewhere just the cwd itself.
  const projects = {};
  for (const key of trustKeysFor(project)) projects[key] = { hasTrustDialogAccepted: true };
...
// THE CLI CHANGED ITS PROJECT KEY SPELLING UNDER US. Claude Code 2.1.229 keys
// `projects` by the FORWARD-slash cwd on Windows (`C:/dev/x`), where 2.1.226
// used backslashes; verified empirically on 2026-08-13: a fresh accept created
// the forward-slash entry, and a backslash entry holding
// `hasTrustDialogAccepted: true` was simply never read. Every folder the user
// had ever trusted re-prompted after the CLI auto-updated, and because the
// same first-run flow carries the bypass-permissions acceptance, each showed
// TWO dialogs. Trust therefore goes under both spellings: forward-slash for
// the current CLI, backslash for a not-yet-updated one; an extra unread entry
// is inert.
test('trustKeysFor covers every spelling the CLI reads', () => {
  if (process.platform === 'win32') {
    assert.deepEqual(trustKeysFor('C:\\dev\\proj'), ['C:\\dev\\proj', 'C:/dev/proj']);
    assert.deepEqual(trustKeysFor('C:/already/forward'), ['C:/already/forward']);
  } else {
    assert.deepEqual(trustKeysFor('/home/you/dev/proj'), ['/home/you/dev/proj']);
  }
});

test('a folder trusted only under the backslash key gains the forward-slash twin', { skip: process.platform !== 'win32' }, (t) => {
  const { home, project } = box(t);
  const configHome = path.join(home, '.claude');
  fs.mkdirSync(configHome, { recursive: true });
  const file = path.join(configHome, '.claude.json');
  // The pre-update state every real config home was in: backslash key trusted,
  // forward-slash key absent. The CLI reads only the latter, so this folder
  // WOULD re-prompt; preaccept must close that gap.
  ...
  const after = JSON.parse(fs.readFileSync(file, 'utf8'));
  assert.equal(after.projects[project.replace(/\\/g, '/')].hasTrustDialogAccepted, true, 'the key the CLI reads');
  assert.equal(after.projects[project].hasTrustDialogAccepted, true, 'the legacy key survives');
});
