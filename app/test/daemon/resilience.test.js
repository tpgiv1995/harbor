...
// The win32 port (2026-08-30). Three things made this suite HANG the whole
// family run on Windows rather than fail: spawnSync of a shebang script is not
// executable on win32 (the CLI must run through the node runner); the daemon
// binds a NAMED PIPE derived by paths.js listenAddress, so an fs.existsSync
// probe on the socket PATH is false forever; and when that probe threw, the
// daemon's cleanup had not been registered yet, so the leaked child kept the
// test process's handle set alive and node --test waited on it without limit.
// Cleanup now registers BEFORE the readiness wait, readiness is a real connect
// probe, and every fixture listener binds through listenAddress.
...
async function listeningServer(handler) {
  ...
  await new Promise((resolve) => server.listen(listenAddress(socket), resolve));
  cleanups.push(async () => {
...
async function runningDaemon(extraEnv = {}) {
  ...
  await waitUntil(() => canConnect(socket), 10000);
...
test('wedged socket times out status and list while logs falls back, and healthy requests still answer', async () => {
  ...
  for (const command of ['status', 'list']) {
    ...
    const result = spawnSync(guiNodeExec, [CLI, command], { env, encoding: 'utf8', timeout: 2000 });
  ...
  const logs = spawnSync(guiNodeExec, [CLI, 'logs', '--lines', '5'], { env, encoding: 'utf8', timeout: 2000 });
  ...
  for (const command of ['status', 'list', 'logs']) {
    const result = spawnSync(guiNodeExec, [CLI, command], { env: healthy.env, encoding: 'utf8', timeout: 2000 });
...
test('daemon to keeper requests time out, while the same screen request succeeds for a healthy keeper', async () => {
  ...
  // Terminate through the daemon so the pty tree dies properly: a SIGKILLed
  // keeper strands its ConPTY cmd.exe, whose cwd is the store dir, and a live
  // cwd makes the cleanup rmSync EPERM forever on Windows.
