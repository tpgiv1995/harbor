...
// win32 binds a NAMED PIPE derived from the socket path, so the socket FILE
// never exists there; daemon-up is proven by connecting, not by statting.
const canConnect = (socketPath) => new Promise((resolve) => {
...
test('a harness daemon dies with its harness, and takes its sessions along', { timeout: 30000 }, async () => {
  ...
  const harness = spawn(guiNodeExec, ['-e', 'setInterval(() => {}, 1000);'], { stdio: 'ignore' });
  ...
  try {
    for (let i = 0; i < 100 && !(await canConnect(socket)); i += 1) await sleep(50);
    assert.ok(await canConnect(socket), 'daemon should come up');
    ...
    fs.rmSync(store, { recursive: true, force: true, maxRetries: 20, retryDelay: 100 });
  }
});
...
test('a relocated daemon with no explicit answer watches its spawner (the suites need no edits)', { timeout: 30000 }, async () => {
  ...
  const intermediate = spawn(guiNodeExec, ['-e', `
    const { spawn } = require('node:child_process');
    const child = spawn(${JSON.stringify(guiNodeExec)}, [${JSON.stringify(DAEMON)}], { windowsHide: true,
      env: { ...process.env,
