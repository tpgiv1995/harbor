...
test('1. the orphan test: pattern scope, recycled pids, and unreadable facts all refuse correctly', () => {
  ...
  const table = T([
    ...
    // 2026-09-04: test fixture servers a session's bash tool left behind are
    // managed; a hand-started dev server in the same language is not.
    { pid: 900, ppid: 999, cmd: 'C:\\Users\\x\\AppData\\Local\\Programs\\Python\\Python312\\python.exe test/ui-screenshots/mock_server.py 8797', created: 50 }, // orphan fixture
    { pid: 901, ppid: 999, cmd: 'C:\\Users\\x\\AppData\\Local\\Programs\\Python\\Python312\\python.exe -m http.server 8793 --bind 127.0.0.1', created: 50 },  // orphan fixture
    { pid: 902, ppid: 999, cmd: '"C:\\dev\\Data-Mapper-Live\\.venv\\Scripts\\python.exe" render.py --serve', created: 50 },                                // orphan, NOT managed (a dev server)
    { pid: 903, ppid: 100, cmd: 'C:\\Users\\x\\AppData\\Local\\Programs\\Python\\Python312\\python.exe test/ui-screenshots/mock_server.py 8790', created: 60 }, // fixture, parent alive -> NOT orphan
  ]);
  const orphans = findManagedOrphans(table, DEFAULT_PATTERNS);
  assert.deepEqual(orphans.map((o) => o.pid).sort(), [200, 400, 900, 901],
    'exactly the managed true orphans: never the unmanaged one, the parented one, the dev server, or the unreadable ones');
});
